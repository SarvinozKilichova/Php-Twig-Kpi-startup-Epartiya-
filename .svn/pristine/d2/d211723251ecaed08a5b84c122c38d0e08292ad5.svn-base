...
class Application_Lib_Varien_Object implements ArrayAccess
{
        ...
        if (strpos($key,'/')) {
            ...
            foreach ($keyArr as $i=>$k) {
                ...
                if (is_array($data)) {
                    ...
                    $data = $data[$k];
                } elseif ($data instanceof Application_Lib_Varien_Object) {
                    $data = $data->getData($k);
                } else {
                    return $default;
                }
        ...
        if (isset($this->_data[$key])) {
            ...
            if (is_array($value)) {
                ...
                return (isset($arr[$index]) && (!empty($arr[$index]) || strlen($arr[$index]) > 0))
                    ? $arr[$index] : null;
            } elseif ($value instanceof Application_Lib_Varien_Object) {
